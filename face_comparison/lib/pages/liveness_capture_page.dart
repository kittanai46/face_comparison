...
class _LivenessCaptureViewState extends State<LivenessCaptureView> {
  ...
  static const _turnHoldDuration = Duration(seconds: 1);
  ...
  DateTime? _firstTurnHoldStartedAt;
  DateTime? _secondTurnHoldStartedAt;
  ...
  void _resetCurrentStageSamplesForDistance() {
    switch (_step) {
      ...
      case _LivenessStep.turnOne:
        ...
        _firstTurnSign = 0;
        _firstTurnHoldStartedAt = null;
        break;
      case _LivenessStep.turnTwo:
        _secondTurnParallaxSamples.clear();
        _secondTurnHoldStartedAt = null;
        break;
      default:
        break;
    }
  ...
  void _handleTurnOne(Face face) {
    ...
    if (!_turnNeutralReady) {
      if (angleY.abs() < _neutralAngleThreshold) {
        ...
        if (_neutralParallaxSamples.length >= _parallaxSamplesRequired) {
          ...
          setState(() {
            _instruction =
                'ค่าหน้าตรงพร้อมแล้ว กรุณาหันไปด้านหนึ่งและค้างไว้ 1 วินาที';
          });
        }
      } else {
    ...
    if (angleY.abs() > _turnAngleThreshold) {
      if (_firstTurnSign == 0 || angleY.sign != _firstTurnSign) {
        _firstTurnSign = angleY.sign;
        _firstTurnHoldStartedAt = DateTime.now();
        _firstTurnParallaxSamples.clear();
      }
      _firstTurnHoldStartedAt ??= DateTime.now();
      if (_firstTurnParallaxSamples.length < _parallaxSamplesRequired + 7) {
        _firstTurnParallaxSamples.add(parallax);
      }
      _showTurnHoldProgress(
        _firstTurnHoldStartedAt!,
        _firstTurnParallaxSamples.length,
      );
      return;
    }
    if (_firstTurnSign != 0 && angleY.abs() < _neutralAngleThreshold) {
      if (_turnHoldCompleted(
        _firstTurnHoldStartedAt,
        _firstTurnParallaxSamples.length,
      )) {
        ...
        setState(() {
          ...
          _instruction = 'ดีมาก กรุณาหันไปอีกด้านหนึ่งและค้างไว้ 1 วินาที';
        });
      } else {
        _firstTurnSign = 0;
        _firstTurnHoldStartedAt = null;
        _firstTurnParallaxSamples.clear();
        _setTurnInstruction(
          'กลับมามองตรงเร็วเกินไป กรุณาหันใหม่และค้างไว้ 1 วินาที',
        );
      }
    }
  }
  ...
  void _handleTurnTwo(Face face) {
    ...
    if (oppositeDirectionReached) {
      _secondTurnHoldStartedAt ??= DateTime.now();
      if (_secondTurnParallaxSamples.length < _parallaxSamplesRequired + 7) {
        _secondTurnParallaxSamples.add(parallax);
      }
      _showTurnHoldProgress(
        _secondTurnHoldStartedAt!,
        _secondTurnParallaxSamples.length,
      );
      return;
    }
    if (_secondTurnHoldStartedAt != null &&
      ...
      _secondTurnParallaxSamples.clear();
      _setTurnInstruction(
        'กลับมามองตรงเร็วเกินไป กรุณาหันอีกด้านใหม่และค้างไว้ 1 วินาที',
      );
      return;
    }
    if (_turnHoldCompleted(
  ...
  bool _turnHoldCompleted(DateTime? startedAt, int sampleCount) {
    return startedAt != null &&
        sampleCount >= _parallaxSamplesRequired &&
        DateTime.now().difference(startedAt) >= _turnHoldDuration;
  }

  void _showTurnHoldProgress(DateTime startedAt, int sampleCount) {
    final elapsed = DateTime.now().difference(startedAt);
    final remainingMilliseconds = max(
      0,
      _turnHoldDuration.inMilliseconds - elapsed.inMilliseconds,
    );
    final String message;
    if (remainingMilliseconds > 0) {
      final remainingSeconds = remainingMilliseconds / 1000;
      message =
          'ค้างท่านี้ไว้อีก ${remainingSeconds.toStringAsFixed(1)} วินาที';
    } else if (sampleCount < _parallaxSamplesRequired) {
      message = 'ค้างต่ออีกเล็กน้อย ระบบกำลังเก็บข้อมูลใบหน้า';
    } else {
      message = 'ค้างครบ 1 วินาทีแล้ว กรุณากลับมามองตรง';
    }
    _setTurnInstruction(message);
  }

  void _setTurnInstruction(String message) {
    if (!mounted || _instruction == message) return;
  ...
  String _normalInstructionForStep() {
    return switch (_step) {
      ...
      _LivenessStep.turnOne =>
        'กรุณาหันหน้าไปด้านหนึ่ง ค้างไว้ 1 วินาที แล้วกลับมามองตรง',
      _LivenessStep.turnTwo =>
        'กรุณาหันหน้าไปอีกด้าน ค้างไว้ 1 วินาที แล้วกลับมามองตรง',
